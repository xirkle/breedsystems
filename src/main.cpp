...
#if defined(NDEBUG)
# error "BreedSystems cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7813309c634f3f3e317a06985d10bc8f6a98c5c4c2552aa3c7bb615f71eb5569");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BreedSystems: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BreedSystems Signed Message:\n";
...
{
    // BreedSystems: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 195 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 187200000 * COIN;
            }
...
static const int64 nTargetTimespan = 12960 * 60;
static const int64 nTargetSpacing = 720 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BreedSystems: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BreedSystems: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1532318743, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Malta wants to become Blockchain Island";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 195 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044a939e662748d232fc8942dc6916186d8aa89b76b1b27518e1f3db90a860337cf687730510e64fe4cfdff7ea0d9a464a20ce5d6396b3cf474fb64ded224f24ab") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1532318743;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1355046;
        ...
        assert(block.hashMerkleRoot == uint256("0xb6fde826c623dc5b8d5dd11eef70f4768fc585169bad83932cd4f8952426a39c"));
...
unsigned char pchMessageStart[4] = { 0xb8, 0x3a, 0x46, 0x8f }; // BreedSystems: increase each by adding 2 to bitcoin's value.
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // transactions:
            if (!fSortedByFee &&
                ((nBlockSize + nTxSize >= nBlockPrioritySize) || (dPriority < COIN * 2 / 250)))
            {
                fSortedByFee = true;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BreedSystemsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BreedSystemsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BreedSystemsMiner(CWallet *pwallet)
{
    printf("BreedSystemsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("breedsystems-miner");
    ...
    try { loop {
        ...
        printf("Running BreedSystemsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BreedSystemsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BreedSystemsMiner, pwallet));
}
